...
- **Text it produces** is text only. Every literal in generated advice (prepared-transaction GIDs, slot names) is a single physical line: quotes and backslashes are doubled and line breaks and other control characters are written as escapes, so text inside a name cannot end one of the `--` comments that keep destructive statements inert. Table names in `vacuum_sql` are quoted with `quote_identifier`. Names shown in messages have control characters escaped. Text copied into fixed buffers is clipped on a character boundary.
- **Explanations** (the `reason` field, the severity explanation in `pg_horizon_check.message` and `pg_horizon_report()`) do not name the pid or state of a session the caller cannot see.
